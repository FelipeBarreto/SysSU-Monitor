...
public class AODV extends IRouter{
		...
		try {
			if(message.getType().equalsIgnoreCase(AODVMessage.AODV_MESSAGE))
			{
				...
				if(message.getType().equalsIgnoreCase(UserDataMessage.class.getCanonicalName()))
				{
					...
					if(getMyAddresses().contains(message.getDestination())){
	...
	@Override
	public void routeMessage(Device destinationDevice, JSONObject message)
			...
			dataMessage.setDestinationAddress(destinationDevice);
			
			if(IRouter.destinationIsBroacast(destinationDevice))
			{
				dataMessage.setNextHop(destinationDevice);
			}
	...
	private boolean iAmTheDestination(RouteRequest routeRequest) {
		return getMyAddresses().contains(routeRequest.getDestination());
	}

	private boolean createOrUpdatePathTo(Device destination, int destinationSequenceNumber,
			Device nextHop, int hopCount){
		
		
		try {
			
				return routingTable.updateRouteTo(destination, destinationSequenceNumber,
						nextHop, hopCount);
		} catch (InvalidRouteException e) {
			e.printStackTrace();
			sender.onNewNodeReachable(destination);
			return true;
		} catch (NoSuchRouteException e) {
			//e.printStackTrace();
			if(routingTable.createForwardRouteEntry(destination, destinationSequenceNumber,
					nextHop, hopCount))
			{
				sender.onNewNodeReachable(destination);
				return true;
	...
	private boolean sendRREQ(Device destinationAddress, int lastDestinationSequenceNumber) {
	...
	{	
		if(iAmTheDestination(rreq))
		{
			...
			return new RouteReply(rreq.getDestination(), node.getCurrentSequenceNumber(),
					rreq.getSourceAddress(), rreq.getSourceSequenceNumber());
			
		} else {
				ForwardRouteEntry entry = routingTable.getForwardEntry(rreq.getDestination());
				...
				return new RouteReply(rreq.getDestination(), destinationSequenceNumber,
	...
	private void receiveRREP(RouteReply routeReply) throws NoSuchRouteException, InvalidRouteException {
		...
		if(createOrUpdatePathTo(routeReply.getDestination(), routeReply.getDestinationSequenceNumber(),
				routeReply.getPreviousHop(), routeReply.getHopCount()) == true)
		{
			if(!getMyAddresses().contains(routeReply.getSourceAddress()))
			{							
					Device nextHop = routingTable.getPathToDestination(routeReply.getSourceAddress());
					routingTable.addPrecursorNode(routeReply.getDestination(), nextHop);
	...
	private void sendGratuitousReply(RouteRequest rreq, int destinationSequenceNumber) throws NoSuchRouteException, InvalidRouteException{
		...
		rreq.getDestination(), destinationSequenceNumber, gratuitousHops, lifetime);
		getSender().queueRoutingMessage(rrepGratuitous);
	}	

	

	public Device getNextHopTo(Device destinationAddress) throws NoSuchRouteException, InvalidRouteException {
	...
	public void OnInvalidRouteOccur(Device destinationAddress)
	{
		notifier.notifyAboutInvalidRouteTo(destinationAddress);
	}

	public int getLastKnownSequenceNumber(Device destinationAddress) throws NoSuchRouteException{
		return routingTable.getDestinationSequenceNumberTo(destinationAddress);
	}

	void queueRetryRouteRequest(Device destinationAddress,
	...
	void queueRouteErrorMessage(Device unreachableNode,
			int unreachableSequenceNumber, ArrayList<Device> precusors) {
		for (Device destinationAdderss : precusors) {
			RouteError rrer = new RouteError(unreachableNode,
	...
	void queueRouteErrorMessage(Device unreachableNodeAddress,
			int unreachableSequenceNumber, Device destinationAddress) {
		RouteError rrer = new RouteError(unreachableNodeAddress,
				unreachableSequenceNumber, destinationAddress);
		sender.queueRoutingMessage(rrer);		
	}

	void queueNewRouteRequest(Device destinationAddress,
