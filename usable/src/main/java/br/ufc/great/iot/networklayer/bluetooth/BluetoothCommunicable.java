...
public class BluetoothCommunicable extends Communicable {
	...
	class SemcServiceConnection implements ServiceConnection {
		BluetoothListener listener = new BluetoothListener.Stub() {
			...
			@Override
			public void onNotNeighborDeviceFound(String deviceUuid, String deviceName,
			...
			@Override
			public void onDeviceFound(String uuid, String deviceAddress, String deviceName) {
				BluetoothCommunicable.this.onDeviceFound(deviceName, deviceAddress);
			}

			@Override
			public void onDeviceConnect(String uuid, String deviceAddress, String deviceName) {
                Device device = RemoteDevicesManager.getInstance().notifyNewConnection(Tech.BLUETOOTH, uuid, deviceName, deviceAddress);
				BluetoothCommunicable.this.onDeviceConnect(device);
			}

			@Override
			public void onDeviceDisconnect(String uuid, String deviceAddress, String deviceName) {
                Device device = RemoteDevicesManager.getInstance().getDevice(uuid);
                if(device == null){
                    return;
                }
				BluetoothCommunicable.this.onDeviceDisconnect(device);
			}
