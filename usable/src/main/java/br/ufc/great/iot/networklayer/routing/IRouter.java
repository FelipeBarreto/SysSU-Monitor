...
public abstract class IRouter {
	...
	public abstract void routeMessage(Device destination, JSONObject message) throws RoutingException;
	...
	public static boolean destinationIsBroacast(Device destinationAddress)
	{
		return (destinationAddress != null && 
	    (destinationAddress.getBestAddress().equalsIgnoreCase(BROADCAST) ||
	    		destinationAddress.getBestAddress().equalsIgnoreCase(BROADCAST_EXCLUDING_SOURCE)));
		
	}
	public static boolean isBrodcastMessage(RoutingMessage message)
	{
		Device nextHop = message.getNextHop();
