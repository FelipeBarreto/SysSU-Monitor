...
public class NetworkManager{
	 ...
	 * @param-destination
	 */
	public void sendMessage(JSONObject msg, Set<Device> destinations) {
		if (destinations.size() == 1) {
			Device destinationArray[] = new Device[destinations.size()];
			destinationArray = destinations.toArray(destinationArray);
			sendMessage(msg, destinationArray[0]);			
		} else {			
			for (Device destination : destinations) {
	...
	{
        Device d = new Device();
        d.getAddresses().put(Tech.OTHER,IRouter.BROADCAST_EXCLUDING_SOURCE);
		sendMessage(msg, d);
	}
	 ...
	 * @param destination
	 */
	public void sendMessage(JSONObject msg, Device destination)	{
		try {
	...
	private void notifyAboutNeighbor(Device destinationAddress) {
	...
	private final CommunicableEventListener notifier = new CommunicableEventListener() {
        ...
        @Override
        public void onDeviceFound(String deviceAdress, String deviceName) {
            Message m = networkEventHandler.obtainMessage(
                    NetworkEventHandler.DEVICE_FOUND);
            Bundle bundle = new Bundle();
            bundle.putString(NetworkEventHandler.DEVICE_NAME, deviceName);
            bundle.putString(NetworkEventHandler.DEVICE_ADDRESS, deviceAdress);
            m.setData(bundle);
		...
		public void onDeviceConnect(Device device) {
			...
			Bundle bundle = new Bundle();
			bundle.putString(NetworkEventHandler.DEVICE_NAME, device.getName());
			bundle.putString(NetworkEventHandler.DEVICE_ADDRESS, device.getBestAddress());
			m.setData(bundle);
		...
		@Override
		public void updateNeighboorList(Device device,
				String networkID) {
		}
	...
	private final RoutingEventsNotifier routingNotifier = new AODVEventsNotifier() {
		...
		public void onUserDataReceived(UserDataMessage message) {
			...
			bundle.putString(NetworkEventHandler.DEVICE_ADDRESS, message.getSourceAddress().getBestAddress());
			m.setData(bundle);
			networkEventHandler.sendMessage(m);		
		...
		public void onHelloMessageReceived(HelloMessage hello) {
			...
			List<Device> devices = new ArrayList<Device>( hello.getPath());
		...
		public void onNewNodeReachable(Device destinationAddress) {
			if(UUIDFactory.getDeviceUuid().compareTo(destinationAddress.getUuid()) != 0)
			{
		...
		@Override
		public void notifyAboutDestinationUnreachable(Device destinationAddress) {
			notifier.onExceptionOcurred(-1, destinationAddress.getUuid().toString());
		}

		@Override
		public void notifyAboutInvalidRouteTo(Device destinationAddress) {
			notifier.onExceptionOcurred(-2, destinationAddress.getUuid().toString());
		}
