...
public class NetworkObserver extends Thread {
	...
	private void handleIncomingObject(WifiMessage readObject) {
		...
		case HELLO:
			...
			break;
		case HELLO_REPLY:
			mListener.onDeviceConnected(UUIDFactory.getDeviceUuid().toString(), readObject.getSourceIpAddress());
			break;
		case MSG:
			...
			break;
		case BYE:
			mListener.onDeviceDisconnected(UUIDFactory.getDeviceUuid().toString(), readObject.getSourceIpAddress());
			break;
		default:
			break;
		}
