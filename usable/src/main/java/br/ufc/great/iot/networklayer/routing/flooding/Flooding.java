...
public class Flooding extends IRouter {
		...
		if(message.getType().equalsIgnoreCase(FloodingMessage.FLOODING_MESSAGE))
		{
			...
			if(messageTable.addMessage(flodMessage) == true)
			{
				...
				boolean destinationIsBroadcast = 
						IRouter.destinationIsBroacast(flodMessage.getDestination());
				boolean isMine = getMyAddresses().contains(message.getDestination());
				try {
	...
	@Override
	public void routeMessage(Device destination, JSONObject message)
