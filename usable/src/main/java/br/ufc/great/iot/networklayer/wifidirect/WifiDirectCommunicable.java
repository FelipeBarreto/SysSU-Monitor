...
public class WifiDirectCommunicable extends Communicable {
	...
	private WifiDirectListener wifiListener = new WifiDirectListener.Stub() {
		...
		@Override
		public void onDeviceDisconnect(String uuid, String name, String address)
				throws RemoteException {
			...
			WifiDirectCommunicable.this.onDeviceDisconnect(device);
		}

		@Override
		public void onDeviceConnect(String uuid, String name, final String address)
				throws RemoteException {
			onActiveNetwork();
			incrementConnections();
			Log.d(TAG, "onDeviceConnect Called");
            Device device = RemoteDevicesManager.getInstance().notifyNewConnection(Tech.WIFIDIRECT, uuid, name, address);
			WifiDirectCommunicable.this.onDeviceConnect(device);
		}
